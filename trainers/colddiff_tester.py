    ...
    def iterative_sample(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(pbar):
                ...
                use_iterative = not getattr(self.opt, 'no_iterative_sampling', False)
                if self.opt.use_err_cfg:
                    print(f'{i}---err---numview--------')
                    if use_iterative:
                        recon_mu, direct_recon_mu = self.net.iterative_sample_err_noSSIM(sparse_mu, timestep_st)
                    else:
                        recon_mu, direct_recon_mu = self.net.iterative_sample_err_cfg_reset_views(sparse_mu, timestep_st)                        
                else:
                    if use_iterative:
