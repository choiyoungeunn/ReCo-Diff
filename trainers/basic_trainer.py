...
class BasicTrainer:
    ...
    @staticmethod
    def save_checkpoint(param, path, name:str, epoch):
    ...
    def save_model(self, net=None, net_name='', best_val_model= False, ddp_model= False):
        opt = self.opt
        if ddp_model:
            net_param = net.module.state_dict() if net is not None else self.net.module.state_dict() # default multicard
        else:
            net_param = net.state_dict() if net is not None else self.net.state_dict()
        ...
        if best_val_model:
            self.save_checkpoint(net_check, checkpoint_path, self.opt.checkpoint_dir + '-net-' + net_name, 'best')
        else:
    ...
    def load_model(self, net=None, net_checkpath=None, output=False, ddp_model= False):
        ...
        if ddp_model:
            new_checkpoint = {}
            for key, value in net_checkpoint.items():
                new_checkpoint[key.replace('module.', "")] = value
            net_checkpoint = new_checkpoint

        for key, _ in self.net.named_parameters():
            if key in net_checkpoint.keys():
                print('Parameter {} loaded'.format(key))
        if net is None:
            self.net.load_state_dict(net_checkpoint, strict=True)
        else:
            net.load_state_dict(net_checkpoint, strict=True)
    ...
    @staticmethod
    def tensorboard_image(writer, rel_path, step=None, **kwargs):
        for key in kwargs.keys():
            path = os.path.join(rel_path, key)
            global_step = 0 if step is None else step
            writer.add_image(tag=path, img_tensor=kwargs[key], global_step=global_step, dataformats='CHW',)
    ...
    def get_metrics_by_window(self, mu_input, mu_target, to_HU=True, width= 3000, center= 500):
        # calculate mu ct accuracy by CT window
        if to_HU:
            hu_input = self.cttool.window_transform(self.cttool.mu2HU(mu_input), width= width, center= center)
            hu_target = self.cttool.window_transform(self.cttool.mu2HU(mu_target), width= width, center= center)
        else:
    ...
    def load_state_dict_byhand(net, checkpoint, net_depth=[1, 2], checkpoint_depth=[2, 3]):
        ...
        for name, param in params:
            ...
            if num == -1:
                ...
                checkpoint_num = num
                print('loading {} from {}'.format(name, checkpoint_keys[checkpoint_num]))
