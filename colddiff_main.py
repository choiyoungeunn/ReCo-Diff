...
from networks.colddiff import ColdDiffusion, ColdDiffusionErrCFG
...
def get_parser():
    ...
    parser.add_argument('--tensorboard_root', type=str, default='', help='tensorboard root dir')
    parser.add_argument('--tensorboard_dir', type=str, default='', help='tensorboard run dir')
    # wandb config
    ...
    parser.add_argument('--use_err_cfg', action='store_true', default=False, help='use error-conditioned CFG(ReCo-Diff)')
    parser.add_argument('--err_cfg_sigma', type=float, default=1.0, help='tanh normalization sigma for err (0 disables)')
    ...
    parser.add_argument('--no_iterative_sampling', action='store_true', default=False,
                        help='disable iterative sampling in tester (uses sample() when set)')
...
def sparse_main(opt):
    ...
    if opt.use_err_cfg:
        net = ColdDiffusionErrCFG(opt, **wrapper_kwargs)
    else:
